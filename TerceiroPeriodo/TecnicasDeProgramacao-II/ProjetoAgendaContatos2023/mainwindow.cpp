...
{
    ui->setupUi(this);
    agendaDeContatosControle = new jhone::ContatoControle();
    ...
    try {
        std::list<jhone::Contato> *listaDeContatos = 0;
...
void MainWindow::clicarCelula(int row) {
    // Verifique se a linha clicada está dentro dos limites da tabela
    if (row >= 0 && row < ui->tableWidgetListaDeContatos->rowCount()) {
        // Execute a ação desejada com base na célula clicada
        ...
        std::list<jhone::Contato> *listaDeContatos = agendaDeContatosControle->getListaDeContatos();
...
void MainWindow::mostrarContatosNaGrid(std::list<jhone::Contato> *listaDeContatos){
    try {
        // Limpar a grid
        if(listaDeContatos->empty()) ui->tableWidgetListaDeContatos->clearContents();
        ui->tableWidgetListaDeContatos->setRowCount(0);
        while(!listaDeContatos->empty()){
            jhone::Contato objeto = listaDeContatos->front();
...
void MainWindow::on_pushButtonConsultar_clicked()
{
    try {
        ...
        QString cpf = ui->lineEditCPF->text();
        jhone::Contato *objContato = agendaDeContatosControle->consultar(cpf);
