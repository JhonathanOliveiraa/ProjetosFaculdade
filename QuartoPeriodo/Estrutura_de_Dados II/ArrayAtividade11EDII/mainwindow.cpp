...
void MainWindow::on_pushButtonGerar_clicked()
{
    try {
        ...
        clock_t tempoInicialQS = clock();
        ui->textEditOrdenadoQuick->setText(vetor->getConjuntoOrdenadoQuickSort()); // manda ordenar pelo BubbleSort
        clock_t tempoFinalQS = clock();
        double tempoDeRespostaQS = (tempoFinalQS - tempoInicialQS) / static_cast<double>(CLOCKS_PER_SEC) * 1000;
        ui->textEditTempoQS->setText(QString::number(tempoDeRespostaQS) + " Ms");
