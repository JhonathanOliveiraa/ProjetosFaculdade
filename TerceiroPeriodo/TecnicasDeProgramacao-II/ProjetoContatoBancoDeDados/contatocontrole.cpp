...
void ContatoControle::verificarCPF(const Contato &objContato) const{
    if(objContato.getCpf().isEmpty()) throw QString ("ERRO: CPF necessita ser preenchido. Insira um CPF.");
...
void ContatoControle::verificarRegrasIncluir(const Contato &objContato) const{
    if(objContato.getNome().isEmpty()) throw QString ("ERRO: Nome completo necessita ser preenchido. Insira um nome.");
    if(objContato.getEmail().isEmpty()) throw QString ("ERRO: Email necessita ser preenchido. Insira um email.");
    ...
    if(objContato.getEmail().lastIndexOf(".") < objContato.getEmail().indexOf("@")) throw QString ("ERRO: Não existe um email sem nenhum ponto após o @.");
    if(objContato.getEndereco().getLogradouro().isEmpty()) throw QString ("ERRO: Preencha o campo do logradouro.");
    if((objContato.getEndereco().getNumero()).isEmpty()) throw QString ("ERRO: Preencha o campo do número da casa.");
    if(QString::number(objContato.getEndereco().getCep()).isEmpty()) throw QString ("ERRO: Preencha o campo do CEP");
    if(QString::number(objContato.getFone().getDdi()).isEmpty()) throw QString ("ERRO: O DDI precisa ser inserido.");
...
void ContatoControle::incluir (const Contato &objContato){
    try {
...
std::list<Contato> *ContatoControle::getListaDeContatos() {
    return contatoPersistencia->getListaDeContatos();
}

Contato *ContatoControle::consultar (QString &cpf) {
    return contatoPersistencia->consultar(cpf);
}
void ContatoControle::excluir(QString cpf) {
    if(contatoPersistencia->consultar(cpf)==0) throw QString ("O CPF não existe.");
    contatoPersistencia->excluir(cpf);
}
void ContatoControle::alterar(const Contato &objContato) {
    verificarRegrasIncluir(objContato);
